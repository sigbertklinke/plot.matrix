...
The `plot` command returns invisibly a list of the elements drawn, a bit like `hist`. A `NULL` entry means the element has not been drawn. The polygon data may be useful for placing own elements in the graphic.
...
* the polygon used to draw a matrix element,
* the text drawn for a matrix element, and
* the polygon used to draw a key element.
...
res$key.polygon[[2]]  # NULL since no text was drawn
```
